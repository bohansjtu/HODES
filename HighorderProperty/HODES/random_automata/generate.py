...
from HODES.file.igraph_pickle import write_pickle
...
def generate(
    ...
    """
    generator = randomAutomata(num_states, num_events, min_trans_per_state, max_trans_per_state, det, num_init,
                               num_marked, num_secret, num_uo, num_ua, enforce_accesibility,
                               enforce_max_trans_per_state)
...
class randomAutomata:
    ...
    def validate_parameters(self):
        ...
        if self.num_ua < 0 or self.num_ua > self.num_events:
            raise ValueError("num_ua must be between 0 and num_events")
    ...
    def initialize_automaton(self):
        ...
        # Events are of type Event and are stored as sets
        ids = sample(range(self.num_events), self.num_ua)
        self.g.Eua.update(self.event_names[i] for i in ids)
